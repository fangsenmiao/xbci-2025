...
def csp(data_train, label_train):
    ...
    idx_4 = np.squeeze(np.where(label_train == 4))

    W = []
    for n_class in range(5):
        if n_class == 0:
            idx_L = idx_0
            idx_R = np.concatenate((idx_1, idx_2, idx_3, idx_4))
        elif n_class == 1:
            idx_L = idx_1
            idx_R = np.concatenate((idx_0, idx_2, idx_3, idx_4))
        elif n_class == 2:
            idx_L = idx_2
            idx_R = np.concatenate((idx_0, idx_1, idx_3, idx_4))
        elif n_class == 3:
            idx_L = idx_3
            idx_R = np.concatenate((idx_0, idx_1, idx_2, idx_4))
        elif n_class == 4:
            idx_L = idx_4
            idx_R = np.concatenate((idx_0, idx_1, idx_2, idx_3))

        idx_R = np.sort(idx_R)
        Cov_L = np.zeros([8, 8, len(idx_L)])
        Cov_R = np.zeros([8, 8, len(idx_R)])
    ...
    Wb = np.concatenate((W[0][:, 0:5], W[1][:, 0:5], W[2][:, 0:5], W[3][:, 0:5], W[4][:, 0:5]), axis=1)
