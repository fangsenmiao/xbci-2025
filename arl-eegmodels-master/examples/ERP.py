...
total_data = loadnpyfolder(data_path + 'T')
X  = total_data['data']
y = total_data['label']

test_tmp = loadnpyfolder(data_path + 'E')
X_test  = test_tmp['data']
Y_test = test_tmp['label']

print("data shape: ", X.shape)
print("labels shape: ", y.shape)

kernels, chans, samples = 1, 8, 1992
...
model = EEGNet(nb_classes = 5, Chans = chans, Samples = samples,
...
clf = make_pipeline(
    ...
    Covariances(estimator='lwf'),  # 添加 Ledoit-Wolf 正则化
...
names        = ['audio left', 'audio right', 'vis left', 'vis right', 'stop']
plt.figure(0)
