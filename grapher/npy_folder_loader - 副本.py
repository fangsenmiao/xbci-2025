...
def loadnpyfolder(folder_path):
    ...
    for file_path in file_list:
        ...
        if len(parts) >= 4:
            try:
                ...
                # 确保数据形状为(8, 2000)
                if eeg_data.shape == (2000, 8):
                    eeg_data = eeg_data.T  # 转置为(8, 2000)
                elif eeg_data.shape != (8, 2000):
                    # 处理形状不匹配的情况
                    if eeg_data.shape[0] == 8 and eeg_data.shape[1] > 2000:
                        eeg_data = eeg_data[:, :2000]
                    elif eeg_data.shape[1] == 8 and eeg_data.shape[0] > 2000:
                        eeg_data = eeg_data[:2000, :].T
                    else:
                        # 其他情况用零填充
                        padding = np.zeros((8, 2000))
                        last_valid_values = eeg_data[:, -1]
                        min_dim0 = min(8, eeg_data.shape[0])
                        min_dim1 = min(2000, eeg_data.shape[1])
                        padding[:min_dim0, :min_dim1] = eeg_data[:min_dim0, :min_dim1]
                        for ch in range(8):
                            if min_dim1 < 2000:  # 如果数据长度不足
                                padding[ch, min_dim1:] = last_valid_values[ch]
                        eeg_data = padding

                # 转置为(2000, 8)并添加到列表
                data_list.append(eeg_data.T)  # 现在形状是(2000, 8)
    ...
    # 堆叠为3维数组(n_samples, 2000, 8)
    data_array = np.stack(data_list, axis=0)  # 形状(n, 2000, 8)
