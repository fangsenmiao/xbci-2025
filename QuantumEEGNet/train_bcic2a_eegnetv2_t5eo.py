...
def getAllDataloader(subject, ratio, data_path, bs):
    ...
    x_train = x_train[:, :, :, 498:1976].to(dev)
    y_train = y_train.long().to(dev)
    x_valid = x_valid[:, :, :, 498:1976].to(dev)
    y_valid = y_valid.long().to(dev)
    x_test = x_test[:, :, :, 498:1976].to(dev)
...
class EEGNet(nn.Module):
    def __init__(self, F1=16, D=2, F2=16, dropout_rate=0.25, num_classes=5):
        ...
        self.fc1 = nn.Linear(F1 * D * 178, num_classes)
...
def main():
    ...
    parser.add_argument('--epochs', type=int, default=6000, help='number of epochs to train (default: 150)')
    ...
    parser.add_argument('--output-dir', type=str, default='output_classicaleegnet', help='directory to save metrics and model')
    ...
    model = EEGNet(num_classes=5).to(device)
